...
def compile_c_if_needed():
    c_path = code_exec + ".c"
    if not os.path.exists(c_path):
        print(f"\033[91mMissing source file: {c_path}\033[0m")
        sys.exit(1)

    if not os.path.exists(code_exec) or os.path.getmtime(c_path) > os.path.getmtime(code_exec):
        print(f"\033[94mCompiling {c_path}...\033[0m")
        result = subprocess.run(["gcc", c_path, "-o", code_exec], capture_output=True, text=True)
